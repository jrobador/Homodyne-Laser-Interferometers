...
#define MEM_LENGTH_FILTER 100
#define M_PI 3.14159265358979323846
#define N_BITS_ADC 8
#define X_MEM_LENGTH_ALGORITHM 3
#define ACTUAL_INDEX MEM_LENGTH_FILTER/2 - 1 //el -1 es porque en matlab los arreglos empiezan en 1
#define VELOCITY_TIME_UPDATE 1e-3
#define LIGHT_WAVE_LENGTH 500e-9
#define V_LOCKED 6.25e-6
#define T_S_ADC  4.902e-5
...
float y_mem[MEM_LENGTH_FILTER];
  ...
  void shift_left_memory(float mem[], float newValue) {
      for (int i = 0; i < MEM_LENGTH_FILTER; i++) {
          mem[i] = mem[i + 1];  // Desplazamiento a la izquierda
      }
      mem[MEM_LENGTH_FILTER - 1] = newValue;  // Agregar nuevo dato a la derecha
  }
...
float *slice_array(float *array, int start, int end) {
    ...
    // Asegurarse de que start y end estén dentro de los límites del array
    if (start < 0 || end < start) {
        fprintf(stderr, "Índices de slice no válidos\n");
        exit(EXIT_FAILURE);
    }

    // Allocar memoria para el nuevo array slice
    float *slice = malloc(numElements * sizeof(int));
    if (slice == NULL) {
        perror("Error al asignar memoria para el slice");
        exit(EXIT_FAILURE);
    }

    // Copiar los elementos del array original al nuevo array slice
  ...
  int all_positive_or_all_negative(float arr[]) {
      int all_positive = 1;
      int all_negative = 1;

      for (int i = 0; i < X_MEM_LENGTH_ALGORITHM; i++) {
          if (arr[i] >= 0) {
              all_negative = 0;
          } else {
              all_positive = 0;
...
void cycle_counter_calculation(int *cycle_counter, int *cycle_counter_direction, float x_actual, float y_actual, float x_mem[], float y_mem) {

  if (y_mem < 0 && y_actual > 0) {
      if (all_positive_or_all_negative(x_mem)) {
          if (x_actual >= 0) {
    ...
    if (archivo_1 && archivo_2) {
       ...
       //____________________________ ACA COMIENZA EL CODIGO DEL MICROPROCESADOR_____________________________________________
        ...
        for (int i = 0; i < x_count; i++) {
            ...
            if(reset  || (global_counter > 50 && global_counter < 150 ))
            {
            ...
            else  
            {
                ...
                shift_left_memory(x_mem, x_actual);
                shift_left_memory(y_mem, y_actual);
                ...
                if(counter > ACTUAL_INDEX){
                    ...
                    cycle_counter_calculation(&cycle_counter, &cycle_counter_direction, x_mem[ACTUAL_INDEX], y_mem[ACTUAL_INDEX], slice_array(x_mem, ACTUAL_INDEX - 1 - (X_MEM_LENGTH_ALGORITHM - 1), ACTUAL_INDEX - 1),y_mem[ACTUAL_INDEX - 1] );

                    if (fabs(actual_velocity_mem) <= V_LOCKED) {
                        phi_actual = phase_calculation(dot_product(b, x_mem), dot_product(b, y_mem), cycle_counter);
                    } 
                    else {
