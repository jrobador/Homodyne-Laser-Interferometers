...
#define VELOCITY_TIME_UPDATE 1e-1
...
void Reset_function(void *argument);
...
int cycle_counter=0;
int cycle_counter_mem=0;
float velocity_counter_update=0;
float phi_t0;
float phi_actual;
...
float x_mem_sliced[X_MEM_LENGTH_ALGORITHM];

uint32_t inicio_tiempoTranscurrido;
uint32_t final_tiempoTranscurrido;
uint32_t tiempoTranscurrido;
...
//								sprintf(sent_data, "If cycle counter done\n");
//								HAL_UART_Transmit(&huart1,(uint8_t*)sent_data,strlen(sent_data),10);
						}
				}
				else {
...
int main(void)
{
  ...
  /* Create the thread(s) */
  /* creation of Reset */
  ResetHandle = osThreadNew(Reset_function, NULL, &Reset_attributes);
  ...
  while (1)
  {
	  ...
	  if(counter > ACTUAL_INDEX){
			...
			if (velocity_counter_update * T_S_ADC >= VELOCITY_TIME_UPDATE) {
				...
				if (fabs(actual_velocity) <= V_LOCKED) {
	...
	//			sprintf(sent_data, "%0.9f\n",continuous_displacement);
	//			HAL_UART_Transmit(&huart1,(uint8_t*)sent_data,strlen(sent_data),10);

				velocity_counter_update = 0;
				actual_velocity = (continuous_displacement - continuous_displacement_mem) / VELOCITY_TIME_UPDATE;
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_7, GPIO_PIN_RESET);
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
   ...
   */
	...
	float DC_fall = 0.68491;
	float mean_DC_signal = 2.334916;
	float new_range = mean_DC_signal - DC_fall;

	x_actual = (3.3 * x_adc) / (pow(2,N_BITS_ADC)) - new_range;
	y_actual = (3.3 * y_adc)	/ (pow(2,N_BITS_ADC)) - new_range;

//	sprintf(sent_data, "%1.7f\n",x_actual);
//	HAL_UART_Transmit(&huart1,(uint8_t*)sent_data,strlen(sent_data),10);
//	sprintf(sent_data, "%1.7f\n",y_actual);
//	HAL_UART_Transmit(&huart1,(uint8_t*)sent_data,strlen(sent_data),10);
//	sprintf(sent_data, "%d\n",counter);
...
//		HAL_UART_Transmit(&huart1,(uint8_t*)sent_data,strlen(sent_data),10);
	...
	else
	{
		...
		if(counter > ACTUAL_INDEX){
			...
			for (int i = ACTUAL_INDEX - 1 - (X_MEM_LENGTH_ALGORITHM - 1), j = 0; i <= ACTUAL_INDEX - 1; i++, j++) {
        x_mem_sliced[j] = x_mem[i];
			}
			cycle_counter_calculation(&cycle_counter, &cycle_counter_direction, x_mem[ACTUAL_INDEX], y_mem[ACTUAL_INDEX],x_mem_sliced,y_mem[ACTUAL_INDEX - 1]);
		...
		if(counter == 1)
		{
			inicio_tiempoTranscurrido = HAL_GetTick();
			sprintf(sent_data, "[ %"PRIu32" ]\n",inicio_tiempoTranscurrido);
			HAL_UART_Transmit(&huart1,(uint8_t*)sent_data,strlen(sent_data),10);
		}
		if(counter == 200)
		{
			final_tiempoTranscurrido = HAL_GetTick();
			tiempoTranscurrido = final_tiempoTranscurrido - inicio_tiempoTranscurrido;
			sprintf(sent_data, "[ %"PRIu32" ]\n",tiempoTranscurrido);
			HAL_UART_Transmit(&huart1,(uint8_t*)sent_data,strlen(sent_data),10);
		}
...
/* USER CODE BEGIN Header_Reset_function */
/**
  * @brief  Function implementing the Reset thread.
  * @param  argument: Not used
  * @retval None
  */
/* USER CODE END Header_Reset_function */
void Reset_function(void *argument)
{
  /* USER CODE BEGIN 5 */
  /* Infinite loop */
